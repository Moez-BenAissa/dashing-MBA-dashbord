var TimeKnots = {
    draw: function(id, events, options) {
        ...
        var step = (cfg.horizontalLayout) ? ((cfg.width - 2 * margin - TEXT_LEFT_MARGIN) / (maxValue - minValue)) : ((cfg.height - 2 * margin) / (maxValue - minValue));
        var series = [];
        ...
        svg.selectAll("text")
            ...
            .attr("y", function(d) {
                if (cfg.horizontalLayout) {
                    var y =  Math.floor(cfg.height / 2) + 25
        ...
        svg.selectAll("p")
            ...
            .attr("y", function(d) {
              if (cfg.horizontalLayout) {
                  var x =  Math.floor(cfg.height / 2) - 30
                  return TimeKnots.checkAndFixCollision(svg, ".event-description", x, this);
              }
              var datum = (cfg.dateDimension) ? new Date(d.date).getTime() : d.value;
              var y = Math.floor(step * (datum - minValue) + margin);
